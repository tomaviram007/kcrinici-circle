...
CREATE POLICY "Admins manage feedback forms"
ON public.feedback_forms FOR ALL TO authenticated
...
ALTER TABLE public.event_feedback
  ...
  ADD COLUMN form_id uuid REFERENCES public.feedback_forms(id) ON DELETE CASCADE,
  ADD CONSTRAINT event_feedback_target_chk CHECK (num_nonnulls(event_id, form_id) = 1);
...
AS $function$
  SELECT e.id, e.title, e.event_date FROM public.events e WHERE e.id = _event_id
  UNION ALL
  SELECT f.id, f.title, f.form_date FROM public.feedback_forms f
  WHERE f.id = _event_id AND f.is_active;
$function$;

CREATE OR REPLACE FUNCTION public.submit_event_feedback(_event_id uuid, _anon_id text, _enjoyment smallint, _met_new_person boolean, _new_people_count smallint DEFAULT NULL::smallint, _keep_in_touch boolean DEFAULT false, _keep_in_touch_name text DEFAULT NULL::text, _attend_reason text DEFAULT NULL::text, _preferred_meetup_type text DEFAULT NULL::text, _meaningful_moment text DEFAULT NULL::text, _improvement text DEFAULT NULL::text, _next_event_likelihood smallint DEFAULT NULL::smallint, _nps smallint DEFAULT NULL::smallint)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
